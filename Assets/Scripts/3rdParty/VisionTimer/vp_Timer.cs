...
public class vp_Timer : MonoBehaviour
{
    ...
    //time+callback+interval+constant
    public static void In(float delay, Callback callback, float interval, Handle timerHandle = null)
    {Schedule(delay, callback, null, null, timerHandle, 1, interval,true);}
	...
	public static void In(float delay, ArgCallback callback, object arguments, Handle timerHandle = null,bool constant=false)
	{ Schedule(delay, null, callback, arguments, timerHandle, 1, -1.0f,constant); }

    //time+callback+interval+constant
    public static void In(float delay, ArgCallback callback,float interval, Handle timerHandle = null)
    { Schedule(delay,null,callback, null, timerHandle, 1, interval, true); }

    //time+callback+arguments+interval+constant
    public static void In(float delay, ArgCallback callback, object arguments, float interval, Handle timerHandle = null)
    { Schedule(delay, null, callback, arguments, timerHandle, 1, interval, true); }
	...
	/// </summary>
	private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval,bool ConstantForce=false)
	{
